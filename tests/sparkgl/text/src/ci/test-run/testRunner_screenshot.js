...
// Info about import to be run
var testUrls = []; // See import.json for test urls to be run

var testsFileNameParam = basePackageUri+"/import.json"; // Change name to run custom set of import
if( px.appQueryParams.tests !== undefined) {
  testsFileNameParam = px.appQueryParams.tests;
}

var enableScreenshotParam = undefined;
if( px.appQueryParams.screenshot !== undefined) {
  enableScreenshotParam = px.appQueryParams.screenshot;
}

var dumpScreenShotParam = undefined;
if( px.appQueryParams.dumpScreenShot !== undefined) {
  dumpScreenShotParam = px.appQueryParams.dumpScreenShot;
}
...
// resultsDisplay is for displaying results on screen when all import have completed.
var resultsDisplay;
...
// Widgets for displaying "Running import... " on screen
var mainPage = scene.create({t:"textBox", parent:root, text:"Running import...",pixelSize:25,w:400,h:50,textColor:0x3090ccff,x: 25, y: 25});
...
/** This function is called for each test in testUrls "import"
 *
...
/** This is the main work horse for import.  It is called recursively for
...
var runTests = function( i) {
  ...
  if( i < numUrls) {
      ...
      var screenShotString = "&enableScreenshot=false";
      var enableScreenshot = (undefined != enableScreenshotParam)?enableScreenshotParam:testUrls[i].screenshot;
      if (undefined != enableScreenshot)
      {
        screenShotString = "&enableScreenshot=" + enableScreenshot;
      }

      var dumpScreenShotString = "&dumpScreenShot=false";
      var dumpScreenShot = (undefined != dumpScreenShotParam)?dumpScreenShotParam:testUrls[i].dumpscreenshot;
      if (undefined != dumpScreenShot)
      {
        dumpScreenShotString = "&dumpScreenShot=" + dumpScreenShot;
      }

      var test  = scene.create({t:'scene',parent:root,url:url+manualString+screenShotString+dumpScreenShotString,x:0, y:0, w:scene.w, h:scene.h});
          ...
          testScene.ready.then(function() {
            ...
            //if( testUrls[myIndex].preTest !== undefined) {
             // testScene.api[testUrls[myIndex].preTest]().then( function() {
              ...
              // Still try to iterateTests even if no PreTests were set
                //console.log("starting import without preTests for scene "+myIndex);
                iterateTests(testScene, myIndex,cancelToken);
            }
            ...
            if(testUrls[myIndex].timeToRun == undefined) {
              ...
              timeouts[myIndex] = setTimeout( function() {
                    ...
                    runnerResults[testUrls[myIndex].title+".HANGING"]= "FAILURE AND CANCEL TESTS";
                    testScene = null;
                    // start running next set of import
                    //runTests(myIndex+1);
...
//                testScene.remove();
                //testScene = null;
                //timeoutScene = null;
                // Kick off loading of the next url and its import after timeout
...
/** Animate the "..." to indicate that import are running */
...
/** Load the import.json file to get urls and test info to be run */
  var testsFileName = [];
...
//  testsToRun.then(function(data) {
    ...
    // Now, run the import by starting with index 0
  ...
  console.log("Failed to load import file");
  mainPage.text = "Failed to load import file!";
}).catch(function(error) {
  console.log("Error occurred while loading import file: "+error);
});
